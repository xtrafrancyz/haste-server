...
class FileDocumentStore {
	constructor(options) {
		...
		// Create the directory synchronously at initialization
	...
	async cleanup() {
		...
		for (let i = 0; i < files.length; i += BATCH_SIZE) {
			...
			await Promise.allSettled(batch.map(async file => {
				if (this.permanentFiles.includes(file)) {
					return;
				}
	...
	set(key, data, callback, skipExpire) {
		...
		// only used for local files (about.md, etc)
		if (skipExpire) {
			this.permanentFiles.push(md5(key));
		}
