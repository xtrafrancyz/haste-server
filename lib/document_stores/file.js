...
var FileDocumentStore = function(options) {
	...
	this.cleanup();
	setInterval(this.cleanup, 60*60*1000); // one day
};

// Delete old files from data dir
FileDocumentStore.prototype.cleanup = function(){
	var that = this;
	fs.readdir(this.basePath, function(err, files){
		files.forEach(function(file){
			var path = that.basePath + '/' + file;
			fs.stat(path, function(err, stat){
				if (Date.now() - stat.mtime.getTime() > that.expire){
					fs.unlink(path);
